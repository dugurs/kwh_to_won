...
=======
        if energy == 0 :
            energy == 0.001
>>>>>>> 9a6a7c3e6da916198d1e743ee562524f83ed18a1
        ...
        if (iBigFamBoolean or iWelfareDcBoolean) :
            ...
            if (iWelfareDcBoolean) :
                ...
                if (iWelfareDcBoolean == 1) : # B1
                    ...
                    _LOGGER.debug(f'유공자,장애인 : {iWelfareDcValue} = (전기요금계 - 200kWh이하감액 ) or 16,000')
                elif (iWelfareDcBoolean == 2) :
                    iWelfareDcValue = math.floor((UsingCharge + adjustValue) * 0.3)
                    _LOGGER.debug(f'사회복지시설 : {iWelfareDcValue} = (전기요금계 - 필수사용량 보장공제 ) x 30%, 한도 없음')
                elif (iWelfareDcBoolean == 3) :
                    if (iWelfareDcValue > 16000) :
                        iWelfareDcValue = 16000
                    _LOGGER.debug(f'기초생활(생계.의료) : {iWelfareDcValue} = (전기요금계 - 200kWh이하감액 ) or 16,000')
                elif (iWelfareDcBoolean == 4) :
                    if (iWelfareDcValue > 10000) :
                        iWelfareDcValue = 10000
                    _LOGGER.debug(f'기초생활(주거.교육) : {iWelfareDcValue} = (전기요금계 - 200kWh이하감액 ) or 10,000')
                elif (iWelfareDcBoolean == 5) :
                    if (iWelfareDcValue > 8000) :
                        iWelfareDcValue = 8000
                    _LOGGER.debug(f'차사위계층 : {iWelfareDcValue} = (전기요금계 - 200kWh이하감액 ) or 8,000')
            ...
            if (iBigFamBoolean) :
                ...
                if (iBigFamBoolean == 1) : # A1
                    if (iBigFamValue > 16000) :
                        iBigFamValue = 16000
                    _LOGGER.debug(f'대가족 요금할인 : {iBigFamValue} = 전기요금계({UsingCharge} + {adjustValue}) - {elecBasic200Value} - {iWelfareDcValue_temp} x30%, 최대 16000')
                else :
                    _LOGGER.debug(f'생명유지장치 : {iBigFamValue} = 전기요금계 - {elecBasic200Value} - {iWelfareDcValue_temp} x30%')
